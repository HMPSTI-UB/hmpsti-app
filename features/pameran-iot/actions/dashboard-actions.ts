...
import { db } from "@/db"
import { iot_teams, vote_sessions, votes, merch_products } from "@/db/schema"
import { sql, desc, eq } from "drizzle-orm"
import { getActiveSession } from "./get-teams"

export async function getDashboardStats() {
  const [activeSession, totalTeamsCount, totalVotesCount, totalProductsCount] = await Promise.all([
    getActiveSession(),
